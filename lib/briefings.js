...
export function buildGoalContext(state, goalId = state.currentGoalId) {
  ...
  const taskNodes = enrichedNodes.filter((node) => node.kind !== "goal");
  ...
  return {
    ...
    metrics: {
      ...
      selfDone: taskNodes.filter((node) => node.ownStatus === "done").length,
      selfOngoing: taskNodes.filter((node) => node.ownStatus === "ongoing").length,
      selfNotStarted: taskNodes.filter((node) => node.ownStatus === "not_started").length,
      fullyComplete: taskNodes.filter((node) => node.ownStatus === "done" && (!node.workStatus || node.workStatus === "done")).length,
...
function statusSummary(goal, metrics) {
  const taskWord = metrics.taskTotal === 1 ? "task step is" : "task steps are";
  const ongoingWord = metrics.selfOngoing === 1 ? "is" : "are";
  const notStartedWord = metrics.selfNotStarted === 1 ? "has" : "have";
  const progress = `Quick status update on ${goal.title}: ${metrics.selfDone} of ${metrics.taskTotal} ${taskWord} marked done; ${metrics.selfOngoing} ${ongoingWord} ongoing and ${metrics.selfNotStarted} ${notStartedWord} not started.`;

  if (metrics.fullyComplete === metrics.selfDone) return progress;
  return `${progress} Some completed steps still have downstream work, so ${metrics.fullyComplete} of ${metrics.taskTotal} tracked items are fully complete end to end.`;
}
