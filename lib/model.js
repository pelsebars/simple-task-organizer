...
export function createModel(state, collapsedNodeIds = new Set()) {
  ...
  const visibleNodes = () => currentNodes().filter((node) => !isHidden(node));
  const layoutChildrenOf = (nodeId) => (collapsedNodeIds.has(nodeId) ? [] : childrenOf(nodeId).filter((node) => !isHidden(node)));
  ...
  const successorStreams = (parentId) => {
    const siblings = layoutChildrenOf(parentId);
  ...
  const successorTargetIds = new Set(currentSuccessors().filter((edge) => visibleNodeIds.has(edge.sourceId) && visibleNodeIds.has(edge.targetId)).map((edge) => edge.targetId));
  ...
  return {
    canCollapse: (nodeId) => childrenOf(nodeId).length > 0,
    childStatus,
    childrenOf,
    currentNodes,
    displayStatus,
    ...
    successorOptions,
    treeNumber,
    hiddenDescendantCount: (nodeId) => (collapsedNodeIds.has(nodeId) ? descendantsOf(state, nodeId).size : 0),
    isCollapsed: (nodeId) => collapsedNodeIds.has(nodeId),
    visibleNodes,
    workItemsOf,
    workStatus,
  };
...
export function moveNode(draft, nodeId, mode, targetId) {
  const node = draft.nodes.find((item) => item.id === nodeId);
  const target = draft.nodes.find((item) => item.id === targetId);
  if (!node || !target || node.kind === "goal" || node.goalId !== target.goalId || node.id === target.id) return false;

  const descendants = descendantsOf(draft, node.id);
  if (descendants.has(target.id)) return false;
  if (mode !== "into" && mode !== "after") return false;
  if (mode === "after" && target.kind === "goal") return false;

  const oldParentId = node.parentId;
  detachAndHealSuccessorChain(draft, node);

  if (mode === "into") {
    node.parentId = target.id;
    node.sortOrder = nextSortOrder(draft, target.id);
  } else {
    const targetOutgoing = draft.successors.filter((edge) => edge.sourceId === target.id);
    draft.successors = draft.successors.filter((edge) => edge.sourceId !== target.id);
    node.parentId = target.parentId;
    insertSiblingAfter(draft, node, target);
    addSuccessorEdge(draft, target.id, node.id, node.goalId);
    targetOutgoing.forEach((edge) => addSuccessorEdge(draft, node.id, edge.targetId, node.goalId));
  }

  resequenceChildren(draft, oldParentId);
  resequenceChildren(draft, node.parentId);
  return true;
}

function detachAndHealSuccessorChain(draft, node) {
  const incoming = draft.successors.filter((edge) => edge.targetId === node.id);
  const outgoing = draft.successors.filter((edge) => edge.sourceId === node.id);
  draft.successors = draft.successors.filter((edge) => edge.sourceId !== node.id && edge.targetId !== node.id);

  incoming.forEach((before) => {
    outgoing.forEach((after) => addSuccessorEdge(draft, before.sourceId, after.targetId, node.goalId));
  });
}

function insertSiblingAfter(draft, node, target) {
  const siblings = draft.nodes
    .filter((item) => item.parentId === target.parentId && item.id !== node.id)
    .sort((a, b) => a.sortOrder - b.sortOrder || a.publicId - b.publicId);
...
function addSuccessorEdge(draft, sourceId, targetId, goalId) {
  if (sourceId === targetId) return;
  const exists = draft.successors.some((edge) => edge.sourceId === sourceId && edge.targetId === targetId);
  if (exists || wouldCreateSuccessorCycle(draft, sourceId, targetId)) return;
  draft.successors.push({ sourceId, targetId, goalId });
}
